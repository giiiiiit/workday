import {
  Component, Input,
  Output, EventEmitter, Injectable} from '@angular/core';
// import { NoticeService} from './noticeService'
import { ImgService } from './ImgService'


import { ActionSheetController ,} from "ionic-angular";
import {  ImagePicker, Transfer } from "ionic-native";
import { NoticeService } from "./noticeService";
import { FileChooser } from '@ionic-native/file-chooser';
import { stringify } from '@angular/compiler/src/util';
import { Camera, CameraOptions } from '@ionic-native/camera';
// import { mediaPicker } from '@ionic-native/';

@Injectable()
@Component({
  selector: 'upload-img-video-file',
  templateUrl: `upload-img-video-file.html`,
})

export class uploadIimgVideoFileComponent {

  @Input() serverUrl: string;//服务器地址
  @Input() type: string;//上传类型：img/video/file
  @Input() source: string;//imgPicker/camera/selectSource
  @Input() opacity:string;//图片清晰度
  @Output() successUpload = new EventEmitter();
  @Output() failedUpload = new EventEmitter();


  

  //上传图片-调用相机时传入的参数
   cameraOptImg:any = {
    quality: this.opacity,
    destinationType: 1, // Camera.DestinationType.FILE_URI,
    sourceType: 1, // Camera.PictureSourceType.CAMERA,
    encodingType: 0, // Camera.EncodingType.JPEG,
    mediaType: 0, // Camera.MediaType.PICTURE,
    // allowEdit: true,
    correctOrientation: true
  };
  // 上传图片-调用相册时传入的参数
   imagePickerOptImg:any = {
    maximumImagesCount: 1,//选择一张图片
    width: 800,
    height: 800,
     mediaType: 0, 
    quality: this.opacity,
    sourceType:0
  };

  //上传视频-调用相机时传入的参数
   cameraOptVideo : any = {
    quality: this.opacity,
    destinationType: 1, // Camera.DestinationType.FILE_URI,
    sourceType: 1, // Camera.PictureSourceType.CAMERA,
    encodingType: 0, // Camera.EncodingType.JPEG,
    mediaType: 1, // Camera.MediaType.PICTURE,
    // allowEdit: true,
    correctOrientation: true
  };
  // 上传视频-调用相册时传入的参数
   imagePickerOptVideo : any = {
    selectMode: 101, //101=picker image and video , 100=image , 102=video
    maxSelectCount: 40, //default 40 (Optional)
    maxSelectSize: 188743680, //188743680=180M (Optional)
    mediaType: 1,
    sourceType:0
  };

  //上传插件
  fileTransfer: Transfer;
  upload: any = {
    url: '',           //接收图片的url
    fileKey: 'image',  //接收图片时的key
    headers: {
      'Accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,*/*;q=0.8' //不加入 发生错误！！
    },
    params: {},        //需要额外上传的参数
    success: (data) => { }, //图片上传成功后的回调
    error: (err) => { },   //图片上传失败后的回调
    listen: () => { }   //监听上传过程
  };


  constructor(private notiSer: NoticeService,
   private fileChooser: FileChooser,
    private imgSer: ImgService, private actionSheetCtrl: ActionSheetController, private camera: Camera
  ) {

  }
  ngOnInit() {

    // this.initImgSer();
    // this.imgSer.showPicActionSheet();

  }

  // 启动拍照功能
   startCamera() {
    if(this.type =="img"){
      alert("camera img")

      this.camera.getPicture(this.cameraOptImg).then((imageData) => {
        this.uploadContent(imageData);

      }, (err) => {
        this.notiSer.showToast('ERROR:' + err); //错误：无法使用拍照功能！
      });
    } else if (this.type == "video"){
      alert("camera video")

      this.camera.getPicture(this.cameraOptVideo).then((imageData) => {

        this.uploadContent(imageData);

      }, (err) => {
        this.notiSer.showToast('ERROR:' + err); //错误：无法使用拍照功能！
      });
    }
   
  }

  // 打开手机相册
  private openImgPicker() {
    if (this.type == "img") {
      alert("lib img")
      let temp = '';
      this.camera.getPicture(this.imagePickerOptImg)
        .then((results) => {
          for (var i = 0; i < results.length; i++) {
            temp = results[i];
          }
         
          this.uploadContent(temp);

        }, (err) => {
          this.notiSer.showToast('ERROR:' + err); //错误：无法从手机相册中选择图片！
        });
    } else if (this.type == "video"){
      alert("lib video")
      alert(this.imagePickerOptVideo.mediaType)
      let temp = '';
      this.camera.getPicture(this.imagePickerOptVideo)
        .then((results) => {
          for (var i = 0; i < results.length; i++) {
            temp = results[i];
          }

          this.uploadContent(temp);

        }, (err) => {
          this.notiSer.showToast('ERROR:' + err); //错误：无法从手机相册中选择图片！
        });
    }
   

    /*let str = '{"status":1,"msg":"提示：图片上传成功！","data":"http:\/\/192.168.1.20\/image\/580af6bcc4d40580af6bcc4d45.jpg"}';
    let res = JSON.parse(str);
    this.upload.success(res);*/
  }

  // 使用ionic中的ActionSheet组件
  private useASComponent() {
    let actionSheet = this.actionSheetCtrl.create({
      title: '选择',
      buttons: [
        {
          text: '拍照',
          handler: () => {
            this.startCamera();
          }
        },
        {
          text: '从手机相册选择',
          handler: () => {
            this.openImgPicker();
          }
        },
        {
          text: '取消',
          role: 'cancel',
          handler: () => {

          }
        }
      ]
    });
    actionSheet.present();
  }

  uploadImg(){
    this.initImgSer();
    if (this.source == 'selectSource'){
      this.useASComponent();
    } else if (this.source == 'camera') {
      this.startCamera();
    } else {
      this.openImgPicker();
    }
   
  }
  uploadVideo() {
    this.initImgSer();
    if (this.source == 'selectSource') {
      this.useASComponent();
    } else if (this.source == 'camera'){
      this.startCamera();
    }else{
      this.openImgPicker();
    }
  }
  uploadFile() {
    this.fileChooser.open()
      .then((results) => {
        this.uploadContent(results);

      }, (err) => {
        this.notiSer.showToast('ERROR:' + err); //错误：无法从手机相册中选择图片！
      })
      .catch(e => console.log(e));
  }
  // 初始化上传图片的服务
  private initImgSer() {
  
    this.upload.url = this.serverUrl; // 上传图片的url，如果同默认配置的url一致，那无须再设置
    this.upload.success = (data) => {
      //上传成功后的回调处理
      this.successUpload.emit(data);
    };
    this.upload.error = (err) => {
      this.failedUpload.emit(err);
      this.notiSer.showToast('错误：头像上传失败！');
    };
  }


  //将图片、视频、文件上传到服务器
  uploadContent(path: string) {
    alert("path:"+path)
    if (!path) {
      return;
    }

    this.fileTransfer = new Transfer();

    let options: any;
    options = {
      fileKey: this.upload.fileKey,
      headers: this.upload.headers,
      params: this.upload.params
    };
    this.fileTransfer.upload(path, this.upload.url, options)
      .then((data) => {

        if (this.upload.success) {
          this.upload.success(JSON.parse(data.response));
        }

      }, (err) => {
        if (this.upload.error) {
          this.upload.error(err);
        } else {
          this.notiSer.showToast('错误：上传失败！');
        }
      });
  }
  

}
